...
export function JournalEntryDialog({ open, onOpenChange, onSuccess, entry, vehicleId }: JournalEntryDialogProps) {
  ...
  const handleTemplateChange = (template: string) => {
    ...
    const safetyExpenseAccount = getAccountByCode(ACCOUNTS.VEHICLE_INVENTORY_SAFETY)
    const warrantyExpenseAccount = getAccountByCode(ACCOUNTS.WARRANTY_COSTS)
    ...
    const gasAccount = getAccountByCode(ACCOUNTS.OTHER_OPERATING)
    ...
    const commissionsAccount = getAccountByCode(ACCOUNTS.SALARIES_WAGES)
    const officeAccount = getAccountByCode(ACCOUNTS.OFFICE_SUPPLIES)
    const insuranceAccount = getAccountByCode(ACCOUNTS.INSURANCE)
    const ownerEquityAccount = getAccountByCode(ACCOUNTS.OWNER_EQUITY)
  ...
  const handleVehicleSaleSelect = (vehicleId: string) => {
    ...
    const cashAccount = getAccountByCode(ACCOUNTS.CASH)
    const revenueAccount = getAccountByCode(ACCOUNTS.VEHICLE_SALES)
    const safetyRevenueAccount = getAccountByCode(ACCOUNTS.SERVICE_REVENUE)
    const warrantyRevenueAccount = getAccountByCode(ACCOUNTS.OTHER_REVENUE)
    const omvicRevenueAccount = getAccountByCode(ACCOUNTS.OMVIC_PAYABLE)
    const cogsAccount = getAccountByCode(ACCOUNTS.COGS)
    const inventoryAccount = getAccountByCode(ACCOUNTS.VEHICLE_INVENTORY)
