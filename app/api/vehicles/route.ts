...
export async function POST(request: NextRequest) {
  ...
  // Post the acquisition entry through the shared, balance-checked engine.
  const actingUserId = await resolveActingUserId(supabase)
  try {
    await postVehiclePurchaseEntry(supabase, data, { createdBy: actingUserId })
  } catch (err) {
    if (err instanceof PostingError) {
      // Report the failure instead of returning 201 with no entry written.
      return NextResponse.json(
        {
          data,
          error: `Vehicle created, but the acquisition entry could not be posted: ${err.message}`,
        },
        { status: 422 },
      )
    }
